...
def merkle_assignment():
    ...
    random_leaf_index = random.randint(0, len(primes)) #TODO generate a random index from primes to claim (0 is already claimed)
...
def generate_primes(num_primes):
    ...
    #TODO YOUR CODE HERE
    x = [True] * (num_primes * 15)
    x[0] = x[1] = False # 0 and 1 are not primes
    ...
    for num in range(2, len(x)):
        if x[num]:
            ...
            if len(primes_list) >= num_primes:
                break
            for multiple in range(num * num, len(x), num):
                x[multiple] = False
    return primes_list
