...
def scanBlocks(chain,start_block,end_block,contract_address):
    ...
    #write CSV header, check if it exists
    try:
        with open(eventfile, 'r') as file:
            pass
    except FileNotFoundError:
        with open(eventfile, 'w') as file:
            file.write("chain,token,recipient,amount,transactionHash,address,date\n")
    ...
    if end_block - start_block < 30:
        ...
        for evt in events:
            data = {
                ...
                'address': evt.address,
                'date': datetime.utcfromtimestamp(w3.eth.get_block(evt.blockNumber)['timestamp']).strftime('%m/%d/%Y %H:%M:%S')
            }
